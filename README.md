...
SideQuest is a route-planning app for scenic drives and hidden shortcuts. Build named collections like `Scenic Route` or `No Embarcadero`, add pass-through anchors such as Marina Green, then draw one route that passes through them without treating them as stops.
...
- Route collections and anchors are stored locally in the browser with `localStorage`.
